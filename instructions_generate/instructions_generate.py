...
if __name__ == '__main__':
    ...
    match sys.argv:
        case [this_filename,
              "--generate", ("regions" | "information") as generate_type,
              "--target", ("Metal" | "CUDA") as generate_target,
              "--output", output_file]:
            pass
        case _:
            print("Require: --generate regions|information --target CUDA|Metal --output file_name")
            sys.exit(1)
...
/*
   ...
   This file was automatically generated by {os.path.basename(this_filename)}
*/

"""

    if generate_target == "Metal":
        operations_begin = """constant struct RegionComposition instructions[] = {\n    """
        operation_info_begin = """let instructionsInformation: [(name: String, byteCount: Int, formatType: String)?] = [\n    """
        operation_info_end = """\n]"""
    elif generate_target == "CUDA":
    ...
    for opcode in range(256):
        if opcode in [0x4C, 0xA2, 0x86, 0x20, 0xEA, 0x38, 0xB0, 0x18, 0x90, 0xA9, 0xF0,
                      ...
                      0x3A, 0x5A, 0x7A, 0xDA, 0xFA, 0x80, 0x1C, 0x3C, 0x5C, 0x7C, 0xDC,
                      0xFC,
            ...
            if wire.cycle_base_increment == None:
                # print(wire.cycle_base_increment, addressing)
            ...
            if opcode in [0x04, 0x44, 0x64, 0x14, 0x34, 0x54, 0x74, 0xD4, 0xF4,
                          0x1A, 0x3A, 0x5A, 0x7A, 0xDA, 0xFA, 0x80,
                          0x0C, 0x1C, 0x3C, 0x5C, 0x7C, 0xDC, 0xFC]:
                name = "*NOP"

            if len(name) == 3:
                annotation = f'/*0x{opcode:02X}*//*{name}*/'
            else:
                annotation = f'/*0x{opcode:02X}*//*{name[1:]}*/'

            operations.append(f'{annotation}{region.struct(wire_region=wire, byte_count=pc_increment)}')

            if generate_target == "Metal":
                operation_info.append(f'{annotation}("{name}", {byte_count}, "{format_types[addressing]}")')
            elif generate_target == "CUDA":
                operation_info.append(f'{annotation}OperationInformation{{.name="{name}", .byte_count={byte_count}, .format_type="{format_types[addressing]}"}}')
            ...
            if generate_target == "Metal":
                operation_info.append(f'/*0x{opcode:02X}*//*NOP*/nil')
            elif generate_target == "CUDA":
    ...
    with open(output_file, 'w') as file:
        if generate_type == "regions":
            output = operations_text
        else:
            output = operation_info_text

        file.write(generated_message + output + '\n\n')
