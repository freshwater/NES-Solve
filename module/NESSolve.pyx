...
def run(str file_location,
    ...
    array_hash_sets = np.empty(num_instances*2*65536, dtype=np.uint8)
    cdef char[::1] hash_sets = array_hash_sets
    ...
    NESSolveModule.run(&c_file_location[0], len(file_location),
                       ...
                       num_blocks,
                       &data_lines[0], &hash_sets[0],
                       &frames_red[0], &frames_green[0], &frames_blue[0])

    return array_data_lines, array_hash_sets, array_red, array_green, array_blue
